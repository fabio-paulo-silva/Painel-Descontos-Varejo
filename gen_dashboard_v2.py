...
function renderPromo() {{
  ...
  chartArea('charts-promo-2', [
    ...
    {{id:'ch-p-camp2', title:'R$ por Campanha (Top 15) — role p/ ver todas', scroll:true}},
  ...
  // Campanha por R$ promo (Top 15)
  const byCamp2 = aggregate(rows.filter(r => r[HDR.promo] > 0 && LK.campanha[r[HDR.campanha]]), HDR.campanha, ['promo'])
    .sort((a,b) => b.promo - a.promo).slice(0, 15);
  hbar('ch-p-camp2', byCamp2.map(x => LK.campanha[x.key]), byCamp2.map(x => x.promo), '#4DE8AA', false, null, null);
